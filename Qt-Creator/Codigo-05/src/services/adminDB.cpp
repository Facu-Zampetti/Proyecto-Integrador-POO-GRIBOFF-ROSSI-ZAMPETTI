...
void adminDB::initialize()
{
    ...
    if (!m_db.open()) {
        ...
        qCritical() << "[adminDB] No se pudo abrir la base de datos:" << err;
    ...
    createTables();
    m_initialized = true;
    qInfo() << "[adminDB] Base de datos inicializada en:" << dbPath;
}

void adminDB::shutdown()
{
    if (m_db.isOpen())
        m_db.close();
...
void adminDB::ensureColumnExists(const QString& tableName,
...
bool adminDB::execQuery(QSqlQuery& query) const
{
    if (!query.exec()) {
        qWarning() << "[adminDB] Error SQL:" << query.lastError().text();
        return false;
    }
    return true;
}
...
bool adminDB::saveUserSession(const UserModel& user)
{
    QSqlQuery q(m_db);
...
bool adminDB::loadUserSession(UserModel& outUser)
{
    QSqlQuery q(m_db);
...
bool adminDB::clearUserSession()
{
    QSqlQuery q(m_db);
    return q.exec("DELETE FROM user_session");
}

bool adminDB::hasActiveSession() const
{
    QSqlQuery q(m_db);
...
bool adminDB::saveSetting(const QString& key, const QString& value)
{
    QSqlQuery q(m_db);
    q.prepare(R"(
...
QString adminDB::loadSetting(const QString& key,
...
bool adminDB::cacheVehicle(int userId, const VehicleModel& v)
{
    QSqlQuery q(m_db);
    q.prepare(R"(
...
QList<VehicleModel> adminDB::loadCachedVehicles(int userId, int limit) const
{
...
bool adminDB::cacheAnalysis(const AnalysisModel& a)
{
    QSqlQuery q(m_db);
    q.prepare(R"(
...
AnalysisModel adminDB::loadCachedAnalysis(int vehicleId) const
{
    QSqlQuery q(m_db);
...
bool adminDB::clearUserVehicleCache(int userId)
{
    QSqlQuery q(m_db);
...
Result<QString> adminDB::probeOdbc(const QString& dsn)
{
    if (!QSqlDatabase::drivers().contains("QODBC")) {
        const QString msg = "Driver QODBC no disponible en este kit de Qt.";
        qWarning() << "[adminDB][ODBC]" << msg;
        return Result<QString>::failure(msg);
    }

    const QString connName = "carlens_odbc_demo";
    if (QSqlDatabase::contains(connName))
        QSqlDatabase::removeDatabase(connName);

    QSqlDatabase odbcDb = QSqlDatabase::addDatabase("QODBC", connName);
    odbcDb.setDatabaseName(dsn);

    if (!odbcDb.open()) {
        const QString err = odbcDb.lastError().text();
        QSqlDatabase::removeDatabase(connName);
        qWarning() << "[adminDB][ODBC] No se pudo abrir el DSN «" << dsn << "»:" << err;
        return Result<QString>::failure("No se pudo abrir el DSN «" + dsn + "»: " + err);
    }

    // La conexión de demo se cierra inmediatamente; no sustituye a SQLite.
    odbcDb.close();
    QSqlDatabase::removeDatabase(connName);
    const QString msg = "Conexión ODBC OK al DSN «" + dsn + "».";
    qDebug() << "[adminDB][ODBC]" << msg;
    return Result<QString>::success(msg);
}

bool adminDB::tryOpenOdbc(const QString& dsn)
{
    return probeOdbc(dsn).ok;
}
