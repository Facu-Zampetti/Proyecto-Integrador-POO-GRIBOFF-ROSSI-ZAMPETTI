...
SnapshotPersistenceResult FileSystemSnapshotPersistenceBackend::persist(
    ...
    // Checksum MD5 de la imagen guardada — integridad de la captura.
    const QString checksum = CryptoHelper::md5File(imagePath);
    qDebug() << "[FileSystemPersistence] Captura guardada:"
             << imagePath << "| MD5:" << checksum;
