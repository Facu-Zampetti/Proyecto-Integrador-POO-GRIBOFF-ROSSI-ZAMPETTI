...
YoloProcessController::YoloProcessController(QObject *parent)
    ...
    connect(m_process, &QProcess::readyReadStandardOutput, this, &YoloProcessController::appendStdOut);
    connect(m_process, &QProcess::readyReadStandardError, this, &YoloProcessController::appendStdErr);
    connect(m_process, &QProcess::finished, this, &YoloProcessController::onProcessFinished);
}
...
bool YoloProcessController::start(const QString &source, bool showWindow, QString *errorMessage) {
    ...
    // Las URLs de red (rtsp://, rtmp://, http://...) y los indices de camara (0,1,...)
    // NO son rutas de archivo: deben pasarse tal cual. Solo resolvemos contra la raiz
    // del proyecto las rutas RELATIVAS de archivos locales (p.ej. "Video/autos.mp4").
    const bool isNetworkUrl  = normalizedSource.contains(QStringLiteral("://"));
    const bool isCameraIndex = !normalizedSource.isEmpty()
        && std::all_of(normalizedSource.cbegin(), normalizedSource.cend(),
                       [](QChar c) { return c.isDigit(); });
    if (!isNetworkUrl && !isCameraIndex) {
    ...
    m_projectRoot = resolvedProjectRoot;
    m_stdOutBuffer.clear();

    const QString pythonExe = resolvePythonExe(m_projectRoot);
    const QString scriptPath = resolveScriptPath(m_projectRoot);
...
void YoloProcessController::appendStdErr() {
    const QString data = QString::fromUtf8(m_process->readAllStandardError()).trimmed();
    if (!data.isEmpty()) {
