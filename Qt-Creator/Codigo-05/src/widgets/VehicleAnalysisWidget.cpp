...
void VehicleAnalysisWidget::setupUI()
{
    ...
    m_snapshotLabel->setMinimumSize(180, 130);
    m_snapshotLabel->setMaximumSize(320, 230);
    m_snapshotLabel->setAlignment(Qt::AlignCenter);
    m_snapshotLabel->setStyleSheet(
        "background: #21262d; border: 1px solid #30363d; border-radius: 8px;");
    m_snapshotLabel->setPixmap(QIcon(":/icons/car.svg").pixmap(QSize(72, 72)));
    ...
    auto addAttr = [&](int row, const QString& key, QLabel*& label) {
        ...
        keyLabel->setStyleSheet("color: #484f58; font-size: 12px;");
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        ...
        if (px.loadFromData(reply->readAll()))
            m_snapshotLabel->setPixmap(
                px.scaled(m_snapshotLabel->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
    });
}
