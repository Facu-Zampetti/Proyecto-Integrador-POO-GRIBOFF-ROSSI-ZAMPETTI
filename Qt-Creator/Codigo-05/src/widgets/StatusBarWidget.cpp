...
void StatusBarWidget::setupUI()
{
    ...
    m_messageLabel->setStyleSheet("color: #8b949e; font-size: 13px;");

    m_versionLabel = new QLabel("Carlens v1.0 · YOLOv11 + Gemini AI", this);
    m_versionLabel->setStyleSheet("color: #484f58; font-size: 12px;");
...
{
    m_messageLabel->setText("⚠ " + message);
    m_messageLabel->setStyleSheet("color: #f85149; font-size: 13px;");
    if (durationMs > 0) m_clearTimer->start(durationMs);
}
...
{
    m_messageLabel->setText("✓ " + message);
    m_messageLabel->setStyleSheet("color: #3fb950; font-size: 13px;");
    if (durationMs > 0) m_clearTimer->start(durationMs);
}
