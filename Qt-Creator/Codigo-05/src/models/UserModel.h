...
public:
    ...
    // ── Operadores ────────────────────────────────────────────────────
    /// Igualdad por id — útil para comparar sesiones y buscar en listas.
    bool operator==(const UserModel& other) const;
    bool operator!=(const UserModel& other) const { return !(*this == other); }

    /// Volcado de depuración: qDebug() << user; imprime id/username/email.
    friend QDebug operator<<(QDebug dbg, const UserModel& u);
