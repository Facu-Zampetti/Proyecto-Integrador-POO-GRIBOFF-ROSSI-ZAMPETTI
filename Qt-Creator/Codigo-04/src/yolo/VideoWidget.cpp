...
void VideoWidget::paintEvent(QPaintEvent *event) {
    ...
    painter.drawImage(imageTarget, frame_);

    const int imageWidth = sourceWidth_ > 0 ? sourceWidth_ : frame_.width();
    const int imageHeight = sourceHeight_ > 0 ? sourceHeight_ : frame_.height();
    ...
    for (const DetectionBox &detection : detections_) {
...
void VideoWidget::mousePressEvent(QMouseEvent *event) {
    ...
    for (int index = detections_.size() - 1; index >= 0; --index) {
        if (detections_.at(index).bbox.contains(imagePoint)) {
