...
void SidebarWidget::setupUI()
{
    ...
    QLabel* logo = new QLabel("🎯  Carlens", header);
    QFont logoFont = logo->font();
    logoFont.setPointSize(16);
    logoFont.setWeight(QFont::Bold);
    logo->setFont(logoFont);
    logo->setStyleSheet("color: #58a6ff; letter-spacing: 2px;");
    ...
    m_emailLabel->setStyleSheet("color: #8b949e; font-size: 11px;");
    ...
    auto addBtn = [&](const QString& icon, const QString& text,
                       Carlens::DashboardSection section) {
        QPushButton* btn = createNavButton(text, icon, section);
        m_navGroup->addButton(btn);
        m_navButtons[section] = btn;
        navLayout->addWidget(btn);
    };

    addBtn("📁", "  Cargar Video",   Carlens::DashboardSection::VideoUpload);
    addBtn("📡", "  Stream RTSP",    Carlens::DashboardSection::Rtsp);
    addBtn("📋", "  Historial",      Carlens::DashboardSection::History);
    addBtn("🔍", "  Análisis IA",    Carlens::DashboardSection::Analysis);
...
{
    // Conectar cada botón de navegación a su sección
...
QPushButton* SidebarWidget::createNavButton(const QString& text,
                                             ...
                                             Carlens::DashboardSection /*section*/)
{
    Q_UNUSED(icon) // El icon ya está en el texto por simplicidad
